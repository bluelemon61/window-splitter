...
export default function SplitterWindow({ scale, isVertical, childs = [], address, refs }: Splitter) {
  const windowSize = Math.min(Math.ceil(scale * 100), 100);
  const barSize = 12;
  const foldSize = isVertical ? 32 : 37.367;
  ...
  useEffect(() => {
    if (splitbarDragging > -1) {
      ...
      for (let i = 0; i <= splitbarDragging; i++) {
        const targetEnd = childs[i];
        if ("fold" in targetEnd && targetEnd.fold) {
          endPoint += foldSize + barSize;
        } else {
          endPoint += targetEnd.scale * multiplier + barSize;
        }
        if (i > 1) {
          const targetStart = childs[i - 2];
          if ("fold" in targetStart && targetStart.fold) {
            startPoint += foldSize + barSize;
          } else {
            startPoint += targetStart.scale * multiplier + barSize;
          }
        }
      }
  ...
  return (
    ...
    >
      {splitbarDragging > -1 ? (
        <div className={`absolute flex ${isVertical ? "flex-col" : ""} w-full h-full z-30`}>
          <div
